...
def linear_softmax(X, W, target_index):
    ...
    # Your final implementation shouldn't have any loops
    (loss, dv) = softmax_with_cross_entropy(predictions, target_index)
    dW = np.dot(X.T, dv)
...
class LinearSoftmaxClassifier():
    ...
    def fit(self, X, y, batch_size=100, learning_rate=1e-7, reg=1e-5,
        ...
        for epoch in range(epochs):
            ...
            # and regularization!
            num_butches = len(batches_indices)
            loss_local = []

            for b in range(num_butches):
                (loss, dW) = linear_softmax(X[batches_indices[b]], self.W,
                                            y[batches_indices[b]])
                (loss_l2, dW_l2) = l2_regularization(self.W, reg)

                loss_local.append(loss + loss_l2)
    ...
    def predict(self, X):
        ...
        # Your final implementation shouldn't have any loops
        predict = softmax(np.dot(X, self.W))
